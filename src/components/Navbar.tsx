...
import { NAV_LINKS, WHATSAPP_LINK } from '../constants.tsx';
...
const Navbar: React.FC = () => {
  ...
  const NavItem: React.FC<{ item: typeof NAV_LINKS[number], mobile?: boolean }> = ({ item, mobile = false }) => {
     ...
     if (isAnchor) {
         return (
             <a href={item.href.replace('/', '')} onClick={() => setIsOpen(false)} className={classes} title={item.label}>
                 {item.label}
             </a>
         );
     }
     ...
     return (
         <Link to={item.href} onClick={() => setIsOpen(false)} className={classes} title={item.label}>
             {item.label}
         </Link>
     );
  };
    ...
    <nav className={`fixed w-full z-50 transition-all duration-300 ease-in-out bg-white border-b border-transparent ${scrolled ? 'shadow-md py-2' : 'py-3 md:py-4 shadow-sm'}`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center min-h-[4rem] md:min-h-[5rem]">
          {/* Logo Section */}
          <Link to="/" className="flex-shrink-0 flex items-center gap-2 md:gap-3 cursor-pointer group" title="Voltar para a página inicial">
             <img 
            ...
            <div className="flex flex-col justify-center">
              <span className={`font-extrabold text-lg md:text-xl lg:text-2xl uppercase tracking-tight leading-none text-sancho-primary`}>
                SANCHO
              </span>
