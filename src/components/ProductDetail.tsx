...
const ProductDetail: React.FC = () => {
  ...
  if (!product) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-slate-50 px-4">
        <h2 className="text-3xl font-bold text-sancho-primary mb-4">Produto não encontrado</h2>
  ...
  const productSchema = {
    ...
    "brand": {
      "@type": "Brand",
      "name": product.brand || "SANCHO"
    },
    "category": category?.title,
    "offers": {
      "@type": "Offer",
      "url": `https://sanchotrading.com/produto/${product.id}`,
      "priceCurrency": "MZN",
  ...
  return (
    <div className="bg-slate-50 min-h-screen pt-24 pb-16">
      <SEO 
        title={`${product.name} | Máquinas e Equipamentos`} 
        description={product.fullDescription || `Conheça os detalhes técnicos e especificações do ${product.name}. Soluções de alta performance para importação em Moçambique.`}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-8">
        <nav className="flex items-center text-sm text-slate-500 gap-2 animate-fade-in-up">
          <Link to="/" className="hover:text-sancho-accent" title="Ir para a página inicial" aria-label="Início">Início</Link>
          <span>/</span>
          <Link to="/produtos" className="hover:text-sancho-accent" title="Ir para a página de produtos" aria-label="Produtos">Produtos</Link>
          <span>/</span>
        ...
        <div className="bg-white rounded-3xl shadow-xl overflow-hidden border border-slate-100 opacity-0 translate-y-8 animate-on-scroll">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-0">
            ...
            <div className="p-8 lg:p-12 flex flex-col">
              <h1 className="text-3xl md:text-5xl font-extrabold text-sancho-primary mb-4 leading-tight">
                {product.name}
              </h1>
              
              <div className="w-20 h-1.5 bg-sancho-accent rounded-full mb-8"></div>

              <div className="prose prose-slate mb-8 text-slate-600 text-lg leading-relaxed">
              ...
              <div className="mt-auto grid grid-cols-1 sm:grid-cols-2 gap-4">
                <Link 
                  ...
                  title={`Solicitar cotação para ${product.name}`}
                ...
                <a 
                  ...
                  title={`Falar com um especialista sobre ${product.name} no WhatsApp`}
